using System;
using System.Net;
using System.Windows;
using System.Windows.Controls;
using System.Windows.Documents;
using System.Windows.Ink;
using System.Windows.Input;
using System.Windows.Media;
using System.Windows.Media.Animation;
using System.Windows.Shapes;

namespace EtaSDK.Utils
{
    public static class EtaApiConstants
    {
        public const string EtaApi_ApiKey = "api_key";
        public const string EtaApi_ApiSecret = "api_secret";
        public const string EtaApi_Uuid = "api_uuid";
        public const string EtaApi_Checksum = "api_checksum";
        public const string EtaApi_Timestamp = "api_timestamp";

        public const string EtaApi_Latitude = "api_latitude";
        public const string EtaApi_Longitude = "api_longitude";
        public const string EtaApi_LocationDetermined = "api_locationDetermined";
        public const string EtaApi_Geocoded = "api_geocoded";
        public const string EtaApi_Accuracy = "api_accuracy";
        public const string EtaApi_Ditance = "api_distance";
    }

    public enum EtaParamOptions
    {

    }
}
